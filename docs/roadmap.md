...
- The Domain 1 `terms` block (`content/domains/01-agentic-architecture-orchestration/lesson.ts`) was extracted mechanically from the lesson's own existing prose, at the owner's direction — **pending the owner's review pass**, same as the quiz questions above. Two terms (*Human-in-the-loop (HITL)*, *Stopping conditions*) were already bolded verbatim in the source; five more (*Agent*, *Deterministic workflow*, *Subagent*, *Agent loop*, *Multi-agent architecture*) reuse verbatim sentences that existed but weren't previously called out as terms. **Agent loop** and **Multi-agent architecture** are the two flagged hardest — their definitions assemble fragments from more than one sentence rather than lifting a single one. The two lead-in sentences in the "Human-in-the-loop and stopping conditions" concept block that merely restated HITL/stopping-conditions were deleted (not rewritten) once those terms had a dedicated definition.
